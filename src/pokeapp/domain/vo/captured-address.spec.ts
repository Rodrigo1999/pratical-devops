import CapturedAddress from "./captured-address";
import CustomError from "@shared/domain/service/custom-error";

describe("CapturedAddress", () => {
    it("deve instanciar corretamente com um endereço válido", () => {
        const address = new CapturedAddress("Rua das flores, centro, SAL city, PI, BRA, 64640000");
    ...
    it("deve lançar erro ao instanciar com um endereço inválido (componentes insuficientes)", () => {
        expect(() => new CapturedAddress("Rua das flores, centro, SAL city, PI, BRA")).toThrow(CustomError);
    });

    it("deve lançar erro ao instanciar com um estado inválido (não possui dois caracteres)", () => {
        expect(() => new CapturedAddress("Rua das flores, centro, SAL city, PIA, BRA, 64640000")).toThrow(CustomError);
    });

    it("deve lançar erro ao instanciar com um país inválido (não possui três caracteres)", () => {
        expect(() => new CapturedAddress("Rua das flores, centro, SAL city, PI, BR, 64640000")).toThrow(CustomError);
    });
